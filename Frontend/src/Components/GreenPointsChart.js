...
function GreenPointsChart({inViewPort}) {
  ...
  const getData = (dateFrom, dateUpto, start = 0) => {
    ...
    setLoading(true)
    http.post(`${url}/WebData/getCarbonCredits`, formData)    
      .then(res => {
        if (res.data.status === "Success") {
          var data = res.data.data
          // data.map(item => item.actual_completion_datetime = moment(item.actual_completion_datetime).format("YYYY-MM-DD"))
          data.sort((a, b) => new Date(a.actual_completion_datetime) - new Date(b.actual_completion_datetime))
          var c = []
          var s = []
          var t = 0
          data.map(item => {
            // setCategories([...categories, item.actual_completion_datetime])
            // setSeries([...series, item.sold_weight])
            c.push(item.actual_completion_datetime)
          ...
          setSeries(s)
          if (start == 1) {
            setTotalWeight(t.toFixed(2))
          }

          setTimeWeight(t.toFixed(2))
  ...
  useEffect(() => {
    ...
    // run the getData after 2s of loading complete
  ...
  useEffect(() => {
    console.log("Green Points Chart>>>>", inViewPort)
    var to = moment(new Date).format('YYYY-MM-DD')
    var from = ""
  ...
  useEffect(() => {
    console.log("categories for green points", categories)
...
export default GreenPointsChart